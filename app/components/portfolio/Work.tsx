  ...
  const imageLists = [
    {
      ...
      alt: "ISRブログ",
      title: "ISRブログ",
      overview: "Next.jsのISRブログ。記事はデータベースで管理を行い、ログイン機能にはNext.Auth.jsを利用。Next.jsのServer Actionとprismaを使用しています。",
      ...
      reasonForCreation: "旅程表アプリの集客用ブログとして作成をしました。Vercelにデプロイするので、ISRは他のサーバーだと作成しづらい点から、どうせならとSSGやSSRではなくISRを採用。",
      commitment: "旅程表アプリと同時に作りはじめたので、同じく初作成のアプリとも言えると思います。(こちらの作成の方が時間がかかった)。ダッシュボードも含め、何度も変更を加えたりと、時間がかかりましたがその分、学びも多かったです。",
    ...
    },
    {
      ...
      alt: "SSGブログwithMD",
      title: "SSGブログwithMD",
      overview: "MDファイルを利用して記事を管理することができる、Next.jsのSSGブログ。コンポーネントを変更することで、デザインの変更ができるようになっています。jpegなどpublicフォルダに入れた画像はwebp形式に圧縮変換して生成し、表示することで軽量化されます。",
      technology: "HTML/CSS(TailwindCSS),TypeScript,React,Next.Js14",
      partOfThePackage: "gray-matter,html-react-parser,remark,sharp",
      reasonForCreation: "SSGでのブログだとどれぐらいの表示速度なのかを知りたかった為。また、自作アプリやブログに利用をする為。",
      commitment: "Next.Jsのイメージコンポーネントが使えないので、代替え手段を探してsharpを利用したりと、良い学習の機会にもなったと思います。",
    ...
    },
    {
      ...
      alt: "SSGブログwithMDX",
      title: "SSGブログwithMDX",
      overview: "MDXを利用することでMDを利用しつつ、コンポーネントなどの利用もできる、Next.jsのSSGブログ。画像はsharpを使用し、jpegなどの形式をwebp形式にして表示。MDファイルで記事を管理しつつ、ブログのデザインパーツをコンポーネントで再利用し、簡単に本格的なブログ構築ができます。",
      technology: "HTML/CSS(TailwindCSS),TypeScript,React,Next.js14",
      partOfThePackage: "@mdx-js,gray-matter,next-mdx-remote,react-markdown,remark,sharp",
      reasonForCreation: "MD形式のブログは作成したが、不便な点がありMDXでのブログを作成しました。ブログを運営していくとオリジナルデザインのパーツを作ったりします。しかし、MDだとコンポーネントとして再利用ができません。そのため、MDXにしてコンテンツ内にコンポーネントを組み込めるようにしました。",
      commitment: "公式のドキュメントなどを参考にしましたが、なんだかんだ上手くいかない点などもあり、学びがありました。また、したいと思ったことがなんらかしらの方法で実現できるのは、素晴らしいなと思います。",
    ...
    },
    {
      ...
      alt: "ポートフォリオ",
      title: "ポートフォリオ",
      overview: "SSGで作成。Next.jsのImageコンポーネントの代わりに、sharpで画像を圧縮してwebp形式での画像を表示。",
      technology: "HTML/CSS(TailwindCSS),TypeScript,React,Next.Js14",
      partOfThePackage: "FramerMotion,sharp",
      reasonForCreation: "就職活動の為",
      commitment: "作成したMDXのブログを利用しようかと思いましたが、復習の意味を込めて、あえて一から作成をしました。やはりデザインを考えるのが苦手であり、そしてダサい・・・。",
